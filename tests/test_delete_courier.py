import allure
import requests


from data import Endpoint, Message
from helpers import *


class TestDeleteCourier:
    @allure.title('Удалить курьера')
    def test_delete_courier(self):
        login_pass = register_new_courier_and_return_login_password()
        with allure.step('Запрос на авторизацию курьера отправлен'):
            r = requests.post(Endpoint.LOGIN_COURIER, data={
            'login': login_pass[0],
            'password': login_pass[1]
        })
        courier_id = r.json()['id']
        with allure.step('Запрос на удаление курьера отправлен'):
            r_delete = requests.delete(f'{Endpoint.DELETE_COURIER}{courier_id}')
        assert r_delete.status_code == 200
        assert Message.DELETE_COURIER == r_delete.text


    @allure.title('Удалить несуществующего курьера')
    def test_delete_not_existing_courier(self):
        login_pass = register_new_courier_and_return_login_password()
        with allure.step('Запрос на авторизацию курьера отправлен'):
            r = requests.post(Endpoint.LOGIN_COURIER, data={
            'login': login_pass[0],
            'password': login_pass[1]
        })
        courier_id = 666666
        with allure.step('Запрос на удаление курьера по несуществующему id отправлен'):
            r_delete = requests.delete(f'{Endpoint.DELETE_COURIER}{courier_id}')
        assert r_delete.status_code == 404
        assert Message.DELETE_NOT_EXISTING_COURIER == r_delete.text


    @allure.title('Удалить курьера без id')
    def test_delete_courier_without_id(self):
        with allure.step('Запрос на удаление курьера без его id отправлен'):
            r = requests.delete(Endpoint.DELETE_COURIER)
        assert r.status_code == 404
        assert Message.DELETE_COURIER_WITHOUT_ID == r.text

